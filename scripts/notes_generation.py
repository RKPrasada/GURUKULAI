...
def regenerate_note(exam: str, subject: str, topic: str) -> bool:
    """
    Immediately regenerate a rejected note via LLM and re-queue for NAGA approval.
    Intended to be called in a background thread after rejection.
    Preserves student_id and lang from the original meta so requester info is kept.
    """
    note_path, meta_path = _note_paths(exam, subject, topic)
    meta = _read_meta(meta_path)
    if not meta:
        logger.warning("Notes: regenerate_note — meta not found for %s/%s/%s", exam, subject, topic)
        return False
    ...
    logger.info("Notes: regenerating %s/%s/%s after NAGA rejection", exam, subject, topic)

    # Build an improved prompt that incorporates NAGA's rejection reason
    naga_feedback = meta.get("naga_note", "").strip()
    feedback_line = f"\n\nNAGA feedback on previous version: {naga_feedback}" if naga_feedback else ""

    system = (
        "You are an expert Indian competitive-exam tutor. "
        "Generate well-structured study notes in clear Markdown. "
        "Use ## headings, bullet points, numbered steps, `code` for formulae, "
        "and **bold** for key terms. Make formulae readable in plain text "
        "(e.g. 'Mean = Sum of values / Number of values'). "
        "End with a Quick-Revision box listing key formulae and mnemonics."
    )
    if lang == "hi":
        system += " Respond entirely in Hindi (Devanagari script)."

    prompt = (
        f"Exam: {exam.upper().replace('_', ' ')}\n"
        f"Topic: {topic}\n"
        f"Subtopics to cover: {', '.join(subtopics) if subtopics else 'all core concepts'}\n"
        f"{feedback_line}\n\n"
        "Generate improved, student-friendly study notes. "
        "Focus on clarity, exam-relevant examples, and readable formulae."
    )

    notes = call_gemini(prompt, system)
    if not notes:
        logger.warning("Notes: LLM returned nothing for %s/%s/%s — keeping rejected state", exam, subject, topic)
        return False
    ...
    # Notify NAGA of the fresh version
    try:
        preview = notes[:300].replace("\n", " ")
        get_dabbu().submit_note_for_approval(
            exam=exam, subject=subject, topic=topic,
            note_path=str(note_path), preview=preview,
        )
    except Exception as e:
        logger.warning("Notes: Dabbu notify failed after regeneration: %s", e)

    logger.info("Notes: regenerated %s/%s/%s → pending NAGA re-review", exam, subject, topic)
    return True


def _resolve_requester(student_id: str, source: str) -> str:
    """Return a display name for who requested the notes."""
    if not student_id:
        # Generated by the nightly batch script or Dabbu — not a live student request
        return "Testing / System"
    try:
        users_path = Path(__file__).parent.parent / "data" / "users.jsonl"
        if users_path.exists():
            for line in users_path.read_text(encoding="utf-8").splitlines():
                ...
                if user.get("user_id") == student_id:
                    return user.get("username") or student_id
    except Exception:
        pass
    return student_id
...
def list_pending_notes() -> list[dict]:
    """Return all notes with status=pending, including full content and requester info."""
    pending = []
    for meta_path in sorted(NOTES_DIR.rglob("*.meta.json")):
        ...
        if meta.get("status") == "pending":
            ...
            # Defensive clean: strip skill context suffix from any legacy entries
            raw_topic   = meta.get("topic", "")
            clean_topic = raw_topic.split("\n\n[Skill Context]")[0].split("\n\n[skill_context]")[0].strip()
            pending.append({
                **meta,
                "topic":        clean_topic,
                "content":      content,
                "preview":      content[:400],
                "requested_by": _resolve_requester(student_id, source),
            })
    return pending
